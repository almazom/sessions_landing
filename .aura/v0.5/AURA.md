# AURA SYSTEM PROTOCOL v0.5 — Agent Nexus
...
## 10. EXPECTATION GAP REVIEW MODE

For product work, keep an explicit comparison loop between the current project state and `docs/product/HIGH_LEVEL_EXPECTATIONS.md`.

Required behavior:

- independently compare the product as it exists now against `docs/product/HIGH_LEVEL_EXPECTATIONS.md`
- do this at the start of substantial work, after meaningful milestones, and before declaring the task done
- always produce a visible gap list instead of implying alignment without evidence
- keep the user-facing mode anchored in `what exists now -> what the high-level expectation says -> what gaps remain`

Default comparison frame:

1. current observed behavior
2. relevant expectation from `docs/product/HIGH_LEVEL_EXPECTATIONS.md`
3. concrete gap between them
4. next closure step

Communication rule:

- for product-facing tasks, default to speaking in expectation-gap terms
- do not report progress only as "implemented X"
- also report which expectation is now satisfied and which gaps are still open
- if no explicit gap list can be stated yet, read more before implementing

## 11. AUTONOMOUS FEEDBACK-DRIVEN FULL-CIRCLE ITERATION
...
|---------|------|---------|
| v0.5 | 2026-03-12 | Added recurring comparison against HIGH_LEVEL_EXPECTATIONS with explicit gap-list communication mode |
