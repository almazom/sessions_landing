...
function detailRow(row: DetailRow) {
  return (
    <div key={`${row.label}-${row.value}`} className="rounded-2xl border border-[#d7e0ea] bg-[#f8fbff] p-4">
...
export default function RichSessionCard({
  ...
  const rows = detailRows || [{ label: 'Путь к файлу', value: session.path, emoji: '📁' }];
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
          <div>
            ...
            {actionHref && (
              <Link
                href={actionHref}
                data-testid="latest-session-open-link"
                className="mt-4 inline-flex items-center rounded-full border border-nexus-200 bg-white px-4 py-2 text-sm font-medium text-nexus-700 transition-colors hover:bg-nexus-50"
              >
                ↗ {actionLabel}
              </Link>
            )}
